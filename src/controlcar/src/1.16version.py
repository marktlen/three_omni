...
import Queue
import threading
from controlcar.cfg import CalibrateLinearConfig
...
class BaseController():
		...
		dyn_server=Server(CalibrateLinearConfig,self.dynamic_reconfigure_callback)
            	rospy.loginfo("above is ok.")
		dyn_client = dynamic_reconfigure.client.Client("baseController", timeout=60)
		self.tf_listener = tf.TransformListener()
                self.cmd_vel = rospy.Publisher('/cmd_vel', Twist, queue_size=5)
                twist=Twist()
                self.cmd_vel.publish(twist)
		self.tf_listener.lookupTransform(self.odom_frame_id, self.base_frame_id, rospy.Time(0))
		...
		#------------------------divide line------------------------------
            	...
            	thread1.start()
	    	thread3=threading.Thread(target=self.thread03) #by me##
	    	thread3.setDaemon(True)#by me##
            	thread3.start()#by me##
            	rospy.spin()			
	...
	def get_position(self):
        # Get the current transform between the odom and base frames
		    try:
			(trans, rot)  = self.tf_listener.lookupTransform(self.odom_frame_id,self.base_frame_id, rospy.Time(0))
